...
async function testFullFlow() {
  ...
  try {
    ...
    console.log(
      `   Twilio Account SID: ${accountSid ? '✅ Configurado' : '❌ No configurado'}`,
    );
    console.log(
      `   Twilio Auth Token: ${authToken ? '✅ Configurado' : '❌ No configurado'}`,
    );
    console.log(`   WhatsApp From: ${fromNumber || '❌ No configurado'}`);
    console.log(
      `   Status Callback URL: ${callbackUrl || '⚠️  No configurado (usará configuración global de Twilio)'}`,
    );
    console.log(
      `   Twilio Client: ${twilioService.isInitialized() ? '✅ Inicializado' : '❌ No inicializado'}`,
    );
    ...
    if (pendingInteractions.length === 0) {
      ...
      console.log(
        '   💡 Puedes crear una manualmente o esperar al scheduler job.\n',
      );

      // Opcional: crear una interaction de prueba
      console.log(
        '   ¿Quieres crear una interaction de prueba? (requiere Request ID válido)',
      );
      console.log('   Ejecuta: npm run whatsapp:resend request <request-id>\n');
    } else {
      console.log(
        `   ✅ Encontradas ${pendingInteractions.length} interactions pendientes:\n`,
      );
      pendingInteractions.forEach((interaction, index) => {
        const metadata = interaction.metadata as any;
        console.log(
          `   ${index + 1}. Interaction: ${interaction.id.substring(0, 8)}...`,
        );
        console.log(
          `      Request: ${interaction.request.title || interaction.request.id.substring(0, 8)}`,
        );
        console.log(`      Template: ${interaction.messageTemplate}`);
        console.log(
          `      Scheduled: ${interaction.scheduledFor.toISOString()}`,
        );
    ...
    if (sentInteractions.length === 0) {
      console.log(
        '   ⚠️  No se encontraron mensajes enviados recientemente.\n',
      );
    } else {
      console.log(
        `   ✅ ${sentInteractions.length} mensaje(s) enviado(s) recientemente:\n`,
      );
    ...
    console.log(
      '      - Los webhooks llegarán cuando cambie el estado del mensaje',
    );
    console.log(
      '      - Verifica logs: docker logs -f especialistas-api-dev | grep -i webhook\n',
    );

    console.log('   B) Simular webhook manualmente:');
    console.log(
      '      ./test/scripts/whatsapp/utilities/simulate-webhook.sh status <MessageSid> delivered',
    );
    console.log(
      '      ./test/scripts/whatsapp/utilities/simulate-webhook.sh status <MessageSid> failed\n',
    );
    ...
    if (sentInteractions.length > 0 && sentInteractions[0].twilioMessageSid) {
      ...
      console.log(
        `      ./test/scripts/whatsapp/utilities/simulate-webhook.sh status ${testSid} delivered\n`,
      );
    }
    ...
    const stats = {
      pending: await prisma.requestInteraction.count({
        where: { status: 'PENDING' },
      }),
      sent: await prisma.requestInteraction.count({
        where: { status: 'SENT' },
      }),
      delivered: await prisma.requestInteraction.count({
        where: { status: 'DELIVERED' },
      }),
      responded: await prisma.requestInteraction.count({
        where: { status: 'RESPONDED' },
      }),
      failed: await prisma.requestInteraction.count({
        where: { status: 'FAILED' },
      }),
    };
    ...
    console.log(
      '      docker logs especialistas-api-dev --tail 50 | grep -i "webhook\\|twilio"\n',
    );
    ...
    console.log(
      '   4. Verifica que las interactions se actualizaron correctamente\n',
    );
  } catch (error: any) {
