...
Run the `follow-ups` skill first if you haven't already — it has the domain map (ladders vs.
legacy rules, the guards the scheduler applies, how a reply moves a request's status) that this
recipe assumes. Read `docs/guides/whatsapp/README.md` and
`.claude/rules/05-events-jobs-notifications.md` too.
...
3. **Rule**: preferred path = add a `LadderDef` entry to
   `src/requests/application/follow-up/follow-up-ladders.ts` (status, directions, template,
   days; max 3; optional `appliesTo`/`escalatesWhenUnanswered`). Only write a custom class for
   payloads a ladder can't express. Legacy class form: in
   `src/requests/application/follow-up/rules/<status>-<n>-days.follow-up-rule.ts`:
...
5. **Response handling**: if the new message expects replies that should change request status,
   extend `DetectResponseIntentUseCase` keywords and the transition map in
   `request-interaction-responded.handler.ts`.
6. **Tests**: unit test the rule (`getQuery`, `getTemplate`, `buildPayload`) and, if a repository
...
7. **Docs**: rules table in `docs/guides/whatsapp/README.md`.
