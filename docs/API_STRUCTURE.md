...
| `/api/admin/requests/:id` | `GET` | Detalle completo del request: `client`, `trade`, `provider` unificado (con `trades` para Professional/Company), `interestedProviders`, `isPublic: boolean`, `review: { id, rating, comment, status } \| null` (la única review del request vía `ReviewService.findByRequestId`, `null` si todavía no hay) y `clientRating: number \| null` / `clientRatingComment: string \| null` (la calificación del proveedor sobre el cliente, escalares planos copiados directo de la entidad - independiente de `review` y sin estado de moderación). Sin el chequeo de `canBeViewedBy` de participantes - cualquier admin puede ver cualquier request |
| `/api/admin/requests/:id/status` | `PUT` | Actualizar el estado de un request (body: `{ status: RequestStatus, statusReason?: string (max 500) }`). Los admins bypasean la tabla de transiciones normal (`RequestEntity.canChangeStatusBy` da bypass incondicional a admin), puede mover a cualquiera de los 15 estados salvo a uno de los 11 "provider-required" (`SENT, CONTACT_RELEASED, IN_PROGRESS, FINISHED, CLOSED, UNDER_REVIEW, NOT_COMPLETED, INTERRUPTED, ABANDONED, REJECTED, NO_RESPONSE`) mientras `providerId` sea `null` - 400 `Cannot set status to <status>: no provider is assigned to this request` (mismo guard en `PATCH /requests/:id`). Devuelve el `RequestResponseDto` completo (mismo shape que `PATCH /requests/:id`) |
