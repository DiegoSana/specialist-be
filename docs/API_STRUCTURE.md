...
| `/api/admin/requests/:id` | `GET` | Detalle completo del request: `client`, `trade`, `provider` unificado (con `trades` para Professional/Company), `interestedProviders`, `isPublic: boolean` y `review: { id, rating, comment, status } \| null` (la única review del request vía `ReviewService.findByRequestId`, `null` si todavía no hay). Sin el chequeo de `canBeViewedBy` de participantes - cualquier admin puede ver cualquier request |
