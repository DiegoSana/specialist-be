...
| `GET` | `/admin/requests/:id` | Full request detail: `client`, `trade`, a unified `provider` (with `trades` for Professional/Company), `interestedProviders` (`InterestedProfessionalResponseDto[]`), `isPublic: boolean`, and `review: { id, rating, comment, status } \| null` (the request's single review via `ReviewService.findByRequestId`, `null` if none yet). No participant-only ownership check - any admin can view any request |
