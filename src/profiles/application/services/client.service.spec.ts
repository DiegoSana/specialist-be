...
describe('ClientService', () => {
  ...
  describe('updateProfile', () => {
    ...
    it('should update user profile successfully', async () => {
      ...
      expect(mockUserRepository.update).toHaveBeenCalledWith('user-123', updateDto);
    });
    ...
    it('should validate profilePictureUrl if provided', async () => {
      ...
      await expect(service.updateProfile('user-123', invalidDto)).rejects.toThrow(BadRequestException);
    });

    it('should accept valid profilePictureUrl', async () => {
      const user = createMockUser();
      const updatedUser = createMockUser({ profilePictureUrl: 'https://example.com/photo.jpg' });
  ...
  describe('activateClientProfile', () => {
    it('should activate client profile successfully', async () => {
      ...
      mockPrismaService.client.create.mockResolvedValue({ id: 'client-123', userId: 'user-123' });
    ...
    it('should throw NotFoundException when user not found', async () => {
      ...
      await expect(service.activateClientProfile('non-existent')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ConflictException if client profile already exists', async () => {
      ...
      await expect(service.activateClientProfile('user-123')).rejects.toThrow(ConflictException);
    });
  });
});
